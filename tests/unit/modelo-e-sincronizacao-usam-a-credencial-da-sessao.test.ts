...
describe("o modelo sai pela credencial da sessão", () => {
  ...
  it("pedir o número de OUTRA organização falha fechado e não usa o ambiente", async () => {
    sessoes = {
      [ORG_A]: { phoneNumberId: "111", cifrado: "\\xcifra-a", token: "tok-da-org-a" },
      [ORG_B]: { phoneNumberId: "222", cifrado: "\\xcifra-b", token: "tok-da-org-b" },
    };
    ...
    // O ambiente aponta para OUTRO número e não é credencial coringa: falha
    // fechado sem consultar o espelho nem chamar a Graph.
    await expect(sendTemplateForSession(db, envio(ORG_B, "111"))).rejects.toThrow(
      /^meta_not_configured/,
    );
